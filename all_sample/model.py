...
class BertTextModel_last_layer(nn.Module):
    def __init__(self, args):
        ...
        self.convs = nn.ModuleList(
            [
                nn.Conv2d(
                    ...
                    kernel_size=(k, self.args.hidden_size),
                )
        ...
        self.fc = nn.Linear(
            args.out_channels * len(self.args.filter_sizes), self.args.num_classes
        )  # 一种类型的一个卷积核filter 对应一个 outchannel 特征

    def conv_pool(self, x, conv):
        x = conv(
            x
        )  # shape [batch_size, out_channels, x.shape[1] - conv.kernel_size[0] + 1, 1]
        x = F.relu(x)
        x = x.squeeze(
            3
        )  # shape [batch_size, out_channels, x.shape[1] - conv.kernel_size[0] + 1]
        size = x.size(2)
    ...
    def forward(self, input_ids, attention_mask):  # shape [batch_size, max_len]
        ...
        hidden_out = self.bert(
            input_ids=input_ids,
            attention_mask=attention_mask,
            output_hidden_states=False,
        )
        out = hidden_out.last_hidden_state.unsqueeze(
            1
        )  # shape [batch_size, 1, max_len, hidden_size]
        out = torch.cat(
            [self.conv_pool(out, conv) for conv in self.convs], 1
        )  # shape  [batch_size, self.args.out_channels * len(self.args.filter_sizes]
